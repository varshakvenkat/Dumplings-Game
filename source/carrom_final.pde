import ddf.minim.*;  //to play audio files
AudioPlayer player;
Minim minim;
...
void  calculate_Outer_Coordinates()
{
  /*
...
void boardDisplay(boolean colour)
{
  /*
    ...
    */
    if((x-c.x)*(x-c.x)+(y-c.y)*(y-c.y)<(radius+c.radius)*(radius+c.radius))
    {
      ...
      if((radius+c.radius)*(radius+c.radius)-(x-c.x)*(x-c.x)+(y-c.y)*(y-c.y)>move*move)
        move=int(move*(radius+c.radius)*(radius+c.radius)-(x-c.x)*(x-c.x)+(y-c.y)*(y-c.y));
      return true;
    }
    return false;
  }
...
void mousePressed()
{
  ...
  {
    S.velocity=(int)((mouseY-carrom_board_size)*(-1.3));
    GAME_POSITION++;
  }
